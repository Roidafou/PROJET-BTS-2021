		...
		var config = {
			...
			data: {
				labels: [<?php 
						...
						$sql = 'SELECT * FROM cap_Lumi'; #On choisit quelle table on veut afficher 
						foreach ($conn->query($sql) as $row => $value) {
							print("'");
							print($value['Date_Lumiere']); #colonne de la BDD à afficher 
							print("'");
							print(",");
						}
						$pdo = null;
				...
				datasets: [{
					...
					data: [
						<?php 
						$conn = new PDO('mysql:host=localhost;dbname=Projet_1', 'user', 'azerty'); #connection à la bdd Projet_1
						$sql = 'SELECT * FROM cap_Lumi';
						foreach ($conn->query($sql) as $row => $value) {
							print("'");
							print($value['Valeur_Lumiere']); #colonne de la BDD à afficher
							print("'");
							print(",");
						}
						$pdo = null;
